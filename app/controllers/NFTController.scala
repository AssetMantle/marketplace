...
class NFTController @Inject()(
  ...
  def viewNFT(nftId: String, accountId: String, activeTab: String): EssentialAction = cached(req => utilities.Session.getSessionCachingKey(req), constants.CommonConfig.WebAppCacheDuration) {
    withoutLoginActionAsync { implicit loginState =>
      implicit request =>
        val nft = masterNFTs.Service.tryGet(nftId)
        (for {
          nft <- nft
        } yield Ok(views.html.nft.viewNft(nft,accountId,activeTab))
          ).recover {
  ...
  def overview(nftId: String, accountId: String, activeTab: String): Action[AnyContent] = withoutLoginActionAsync { implicit loginState =>
    implicit request =>
      Future(Ok(views.html.nft.detail.view(nftId = nftId, accountId = accountId, activeTab = activeTab)))
  }
