...
//          val countBuyerNFTsFromSale = masterTransactionBuyNFTTransactions.Service.countBuyerNFTsFromSale(buyerAccountId = loginState.username, saleId = buyNFTData.saleId)
//
//          def collection(id: String) = masterCollections.Service.tryGet(id)
//
//          def nftOwners(collection: Collection) = masterNFTOwners.Service.getRandomNFTsBySaleId(saleId = buyNFTData.saleId, take = buyNFTData.buyNFTs, creatorId = collection.creatorId)
//
//          def checkAlreadySold(nftIds: Seq[String]) = masterTransactionBuyNFTTransactions.Service.checkAlreadySold(saleId = buyNFTData.saleId, nftIds = nftIds)
//
...
//              if (balance == MicroNumber.zero || balance <= (sale.price * buyNFTData.buyNFTs)) Option(constants.Response.INSUFFICIENT_BALANCE) else None,
//              //              if (buyNFTData.mintNFT && (balance - (sale.price + constants.Blockchain.AssetPropertyRate * (nftProperties.length + constants.Collection.DefaultProperty.list.length)) <= MicroNumber.zero)) Option(constants.Response.INSUFFICIENT_BALANCE) else None,
//              if ((countBuyerNFTsFromSale + buyNFTData.buyNFTs) > sale.maxMintPerAccount) Option(constants.Response.MAXIMUM_NFT_MINT_PER_ACCOUNT_REACHED) else None,
...
//                amount = sale.price * buyNFTData.buyNFTs,
...
//            ).recover {
//            case baseException: BaseException => BadRequest(views.html.sale.buySaleNFT(BuySaleNFT.form.withGlobalError(baseException.failure.message), saleId = buyNFTData.saleId))
//          }
//        }
//      )
//  }
