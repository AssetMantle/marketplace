...
object ImageProcess {
  ...
  def convertToThumbnail(filePath: String)(implicit executionContext: ExecutionContext): Array[Byte] = {
    try {
      val file = new File(filePath: String)
      val imageRes = ImageIO.read(file)
      val writer: JpegWriter = JpegWriter.compression(100)
      val bytes = FileOperations.convertToByteArray(scrimage.ImmutableImage.loader().fromFile(file).fit(180, (180 * imageRes.getHeight) / imageRes.getWidth).output(writer, file))
