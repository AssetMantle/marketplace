...
class MicroNumber(val value: BigInt) extends ScalaNumber with ScalaNumericConversions with Ordered[MicroNumber] {
  ...
  def gcd(that: MicroNumber): MicroNumber = if (this.isWhole && that.isWhole) new MicroNumber(this.value.gcd(that.value)) else constants.Response.NUMBER_FORMAT_EXCEPTION.throwBaseException()(MicroNumber.logger)
  ...
  def isProbablePrime(certainty: Int): Boolean = if (this.isWhole) BigInt(this.toLong).isProbablePrime(certainty) else constants.Response.NUMBER_FORMAT_EXCEPTION.throwBaseException()(MicroNumber.logger)
