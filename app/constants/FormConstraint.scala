...
object FormConstraint {
  ...
  val defineCollectionPropertiesConstraint: Constraint[collection.DefineProperties.Data] = Constraint("constraints.DefineCollectionPropertiesConstraint")({ definePropertiesData: collection.DefineProperties.Data =>
    ...
    val errors = Seq(
      ...
      if (definedPropertiesNames.map(_.toLowerCase).intersect(constants.Collection.RestrictedPropertyList.map(_.toLowerCase)).nonEmpty) Option(ValidationError(constants.Response.COLLECTION_PROPERTIES_CONTAINS_RESTRICTED_PROPERTIES.message)) else None,
    ).flatten
    if (errors.isEmpty) Valid else Invalid(errors)
  })
