...
object NFT {
  ...
  case class Property(name: String, `type`: String, `value`: String, meta: Boolean = true, mutable: Boolean = false) {
    ...
    def valid: Boolean = this.`type` match {
      ...
      case constants.NFT.Data.DECIMAL | constants.NFT.Data.NUMBER | constants.NFT.Data.SMALL_NUMBER => Try(BigDecimal(this.`value`)).isSuccess
    ...
    def toBaseNFTProperty: BaseNFTProperty = if (this.valid) this.`type` match {
      ...
      case constants.NFT.Data.DECIMAL | constants.NFT.Data.NUMBER | constants.NFT.Data.SMALL_NUMBER => DecimalProperty(name = this.name, `value` = BigDecimal(this.`value`), meta = this.meta, mutable = this.mutable)
