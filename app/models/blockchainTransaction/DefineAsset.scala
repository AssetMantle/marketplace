...
object DefineAssets {
  ...
  class DefineAssetTable(tag: Tag) extends Table[DefineAsset](tag, "DefineAsset") with ModelTable[String] {
    ...
    def * = (txHash, txRawBytes, fromAddress, status.?, memo.?, timeoutHeight, log.?, createdBy.?, createdOnMillisEpoch.?, updatedBy.?, updatedOnMillisEpoch.?) <> (DefineAsset.tupled, DefineAsset.unapply)
...
class DefineAssets @Inject()(
  ...
  object Service {
    ...
    def add(txHash: String, txRawBytes: Array[Byte], fromAddress: String, status: Option[Boolean], memo: Option[String], timeoutHeight: Int): Future[DefineAsset] = {
      val defineAsset = DefineAsset(txHash = txHash, txRawBytes = txRawBytes, fromAddress = fromAddress, status = status, log = None, memo = memo, timeoutHeight = timeoutHeight)
      for {
