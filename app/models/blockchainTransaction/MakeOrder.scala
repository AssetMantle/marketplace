...
object MakeOrders {
  ...
  class MakeOrderTable(tag: Tag) extends Table[MakeOrder](tag, "MakeOrder") with ModelTable[String] {
    ...
    def * = (txHash, fromAddress, status.?, memo.?, timeoutHeight, log.?, createdBy.?, createdOnMillisEpoch.?, updatedBy.?, updatedOnMillisEpoch.?) <> (MakeOrder.tupled, MakeOrder.unapply)
...
class MakeOrders @Inject()(
  ...
  object Service {
    ...
    def add(txHash: String, fromAddress: String, status: Option[Boolean], memo: Option[String], timeoutHeight: Int): Future[MakeOrder] = {
      val makeOrder = MakeOrder(txHash = txHash, fromAddress = fromAddress, status = status, log = None, memo = memo, timeoutHeight = timeoutHeight)
      for {
        _ <- create(makeOrder)
      } yield makeOrder
    }
