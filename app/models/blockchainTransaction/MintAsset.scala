...
object MintAssets {
  ...
  class MintAssetTable(tag: Tag) extends Table[MintAsset](tag, "MintAsset") with ModelTable[String] {
    ...
    def * = (txHash, fromAddress, status.?, memo.?, timeoutHeight, log.?, createdBy.?, createdOnMillisEpoch.?, updatedBy.?, updatedOnMillisEpoch.?) <> (MintAsset.tupled, MintAsset.unapply)
...
class MintAssets @Inject()(
  ...
  object Service {
    ...
    def add(txHash: String, fromAddress: String, status: Option[Boolean], memo: Option[String], timeoutHeight: Int): Future[MintAsset] = {
      val mintAsset = MintAsset(txHash = txHash, fromAddress = fromAddress, status = status, log = None, memo = memo, timeoutHeight = timeoutHeight)
      for {
        _ <- create(mintAsset)
      } yield mintAsset
    }
