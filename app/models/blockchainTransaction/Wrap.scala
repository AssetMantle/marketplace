...
object Wraps {
  ...
  class WrapTable(tag: Tag) extends Table[Wrap](tag, "Wrap") with ModelTable[String] {
    ...
    def * = (txHash, txRawBytes, fromAddress, status.?, memo.?, timeoutHeight, log.?, createdBy.?, createdOnMillisEpoch.?, updatedBy.?, updatedOnMillisEpoch.?) <> (Wrap.tupled, Wrap.unapply)
...
class Wraps @Inject()(
  ...
  object Service {
    ...
    def add(txHash: String, txRawBytes: Array[Byte], fromAddress: String, status: Option[Boolean], memo: Option[String], timeoutHeight: Int): Future[Wrap] = {
      val wrap = Wrap(txHash = txHash, txRawBytes = txRawBytes, fromAddress = fromAddress, status = status, log = None, memo = memo, timeoutHeight = timeoutHeight)
      for {
        _ <- create(wrap)
      } yield wrap
    }
